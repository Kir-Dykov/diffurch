...
auto Relay1 (double b, double d0, double d1, double tau) {
    array<VecMap<1, 1>, 2> f {
        [b,d0,d1](const Vec<1>& X) {return  Vec<1>{-b*X[0] + d0};},
        [b,d0,d1](const Vec<1>& X) {return  Vec<1>{-b*X[0] + d1};}
    };
    array<VecMap2<1, 1, 1>, 2> df {
        [b,d0,d1](const Vec<1>& X, const Vec<1>& W) {return Vec<1>{-b*W[0]};},
        [b,d0,d1](const Vec<1>& X, const Vec<1>& W) {return Vec<1>{-b*W[0]};}
    };
    
    VecScalarFunc<1> b_ {
        [](const Vec<1>& X) {return X[0];}
    };
    VecMap<1, 1> b_derivative {
        [](const Vec<1>& X) {return Vec<1>{1.};}
    };
    
    Vec<1>  b_levels {0};
    Vec<1>  b_tau {tau};
    
    return DDE_relay(f, df, b_, b_derivative, b_levels, b_tau);
}

// // x'' + b x' + c x = d sign( x_tau )
auto Relay2 (double b,double c, double d, double tau) {
    array<VecMap<2, 2>, 2> f {
        [b,c,d](const Vec<2>& X) {return  Vec<2>{X[1], -b*X[1] - c*X[0] - d};},
        [b,c,d](const Vec<2>& X) {return  Vec<2>{X[1], -b*X[1] - c*X[0] + d};}
    };
    array<VecMap2<2, 2, 2>, 2> df {
        [b,c,d](const Vec<2>& X, const Vec<2>& W) {return  Vec<2>{W[1], -b*W[1] - c*W[0]};},
        [b,c,d](const Vec<2>& X, const Vec<2>& W) {return  Vec<2>{W[1], -b*W[1] - c*W[0]};}
    };
    
    VecScalarFunc<2> b_ {
        [](const Vec<2>& X) {return X[0];}
    };
    VecMap<2, 2> b_derivative {
        [](const Vec<2>& X) {return Vec<2>{1., 0.};}
    };
